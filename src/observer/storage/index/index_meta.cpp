...
RC IndexMeta::init(const std::string &name, const std::vector<const FieldMeta *> &field_metas)
{
  if (name.empty()) {
...
RC IndexMeta::init(const std::string &name, const std::vector<FieldMeta> &field_metas)
{
  if (name.empty()) {
    LOG_ERROR("Failed to init index, name is empty.");
    return RC::INVALID_ARGUMENT;
